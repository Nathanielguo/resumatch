...
> 🧭 **项目定位**：把基础履历与目标 JD 整理为一份可审阅的定制草稿，并将改写依据、差距与面试准备放在同一工作流中。
>
> 🧩 **设计取向**：单文件、无后端、可本地打开；核心是提高针对不同 JD 的整理效率，而不是承诺录用结果。
...
> ⚠️ **使用边界**：匹配评分、关键词命中和 AI 建议均为模型生成的辅助信号，不等同于招聘方或 ATS 的官方评分，也不构成通过筛选的保证。投递前请逐项核对事实、日期和数字。
>
> 🔐 **隐私边界**：工具本身不设中间后端，但生成时仍需将履历、JD 与提示词直接发送至所选 AI 服务商。建议先移除身份证号、住址等非必要敏感信息，并按所用服务商的政策判断是否提交。
